var common = require("../common"),
	...
	MultipartParserStub = GENTLY.stub("./multipart_parser", "MultipartParser"),
...
test(function parse() {
	...
	(function testWithCallback() {
		...
		form.parse(REQ, gently.expect(function parseCbErr(err, fields) {
			assert.strictEqual(err, ERR);
			assert.deepEqual(fields, {
				foo: "bar"
			});
...
test(function _initMultipart() {
	...
	(function testRegularField() {
		...
		gently.expect(form, "onPart", function (part) {
			...
			gently.expect(part, "emit", function (event) {
...
test(function handlePart() {
	...
	(function testUtf8Field() {
		...
		PART.emit("data", new Buffer([226]));
		PART.emit("data", new Buffer([130, 172]));
		PART.emit("end");
	}());
	(function testBinaryField() {
		...
		gently.expect(form, "emit", function (event, field, value) {
			...
			assert.equal(value, "hello world: " + (new Buffer([226, 130, 172])).toString("binary"));
		});
